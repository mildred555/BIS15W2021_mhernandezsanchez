...
2. How does the age of elephants compare by sex? is asking about a range => need a boxplot
```{r}
elephants %>%
...
3. What is the relationship between age and height? two different things that are continuous variables => scatterplot
```{r}
elephants %>%
  ggplot(aes(x=age, y=height))+
  geom_point()+geom_smooth(method = lm, se=F) #se=F makes the confidence interval go away
```
...
homerange %>%
  ...
  geom_bar()+ #geom_bar to show counts
