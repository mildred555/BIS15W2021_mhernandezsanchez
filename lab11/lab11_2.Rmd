...
```{r}
p+theme_linedraw()+
  theme(legend.position = "bottom", #use 'none' to remove legends
        axis.text.x = element_text(angle = 60, hjust=1))+
  labs(title = "Observations by Taxon in Homerange Data",
       x = NULL,
       y= "n",
       fill= "Trophic Guild") #control title of legend (can make legend title lowercase)
```
...
```{r}
p+scale_fill_brewer(palette = "Paired")+ #paired comes from looking at colorbrewer palettes in the bottom right corner using "?RColorBrewer"
  theme(legend.position = "bottom",
        axis.text.x = element_text(angle = 60, hjust=1))+
  labs(title = "Observations by Taxon in Homerange Data",
       x = NULL,
...
```{r}
barplot(rep(1,14), axes=FALSE, col=my_palette) #shows the colors present in the palette
```
...
homerange %>% 
  ...
  geom_point() +
  xlim(0, 4) + #xlim and ylim lets you zoom in on the points within the range you specified
  ylim(1, 6)
```
...
```{r}
p2+ facet_wrap(~taxon) #take plot 2 (above) and group by a variable
```

## Faceting: `facet_grid()`
As you can imagine, there are lots of options for faceting. Another useful facet type is `facet_grid`. 
```{r}
p2+facet_grid(thermoregulation~class) #use grid when using 2 categorical variables
```
