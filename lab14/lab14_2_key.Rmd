...
```{r}
iplotCorr(sug$pheno) #plot correlation between phenotypes
...
```{r}
sug <- calc.genoprob(sug, step=1) #1centimorgan is being used 
out.em <- scanone(sug)
...
#c7.loc45 hihgly related to 6.109 of BP 
```
...
```{r}
iplotScanone(out.em, chr=c(7,15)) #only showing chromo 7 and15
```

```{r}
iplotScanone(out.em, sug) #good to show how the genotype is associated with the phenotype so for HR, BB is strongly associated with HR
```
