...
msleep %>%
  tabyl(genus)
```
**n_distinct works better in this case than tabyl because tabyl looks messy**
...
**There are missing observations in the data so R is treating the whole genera as though they were all NAs, even though some of the genera do, in fact, have measured brain weights.**
```{r}
msleep %>%
...
```
**Cetacea has all NAs, which is why we have a different kind of NA.**
