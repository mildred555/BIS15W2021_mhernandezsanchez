...
## Data
For this homework, we will use a data set compiled by dathe Office of Environment and Heritage in New South Whales, Australia. It contains the enterococci counts in water samples obtained from Sydney beaches as part of the Beachwatch Water Quality Program. Enterococci are bacteria common in the intestines of mammals; they are rarely present in clean water. So, enterococci values are a measurement of pollution. `cfu` stands for colony forming units and measures the number of viable bacteria in a sample [cfu](https://en.wikipedia.org/wiki/Colony-forming_unit).   
...
**No, because the date column has values that should be columns. They are in long format.**
#wide data are easier to enter which is why they're usually not tidy.
</div>
...
sydneybeaches_wide <- sydneybeaches_long %>%
  pivot_wider(names_from = "date", #the observations under date will become new columns
...
sydneybeaches_long_3 <- sydneybeaches_long_2 %>% 
  group_by(year, site) %>%
  summarize(mean_cocci = mean(enterococci_cfu_100ml, na.rm = T), .groups="keep")
...
```r
sydneybeaches_polluted_2018 <- sydneybeaches_long_3 %>%
  filter(year==2018)%>%
  arrange(desc(mean_cocci))
sydneybeaches_polluted_2018
```
