...
```
```{r}
#using pivot longer to make the data more readable
homerange %>%
  filter(class=="mammalia" & trophic.guild=="carnivore") %>%
  summarize(min_log10.mass=min(log10.mass),
            max_log10.mass=max(log10.mass),
...
homerange %>%
  ...
  ggplot(aes(x=family, y=log10.mass))+
  geom_boxplot()+
  coord_flip()#coord_flip put "family" on the y axis so that we could actually read the names of the species.
```
